...
class CreateModerations < ActiveRecord::Migration[5.0]
  def change
    create_table :decidim_moderations do |t|
      t.references :decidim_participatory_process, null: false,
                                                   index: { name: 'decidim_moderations_participatory_process' }
      t.references :decidim_reportable, null: false, polymorphic: true,
                                        index: { unique: true, name: 'decidim_moderations_reportable' }
