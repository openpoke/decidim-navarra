...
class MoveProposalEndorsedEventNotificationsToResourceEndorsedEvent < ActiveRecord::Migration[5.2]
  def up
    Decidim::Notification.where(event_name: 'decidim.events.proposals.proposal_endorsed',
                                event_class: 'Decidim::Proposals::ProposalEndorsedEvent').find_each do |notification|
  ...
  def down
    Decidim::Notification.where(
      ...
      notification.update(event_name: 'decidim.events.proposals.proposal_endorsed',
